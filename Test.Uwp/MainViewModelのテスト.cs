...
namespace Test.Uwp
{
    public class MainViewModelのテスト
    {
        ...
        [Fact]
        public void 分割領域表示の左上は取得できます()
        {
            テスト対象.分割領域の左限 = 100;
            テスト対象.分割領域の上限 = 100;
            テスト対象.元画像倍率パーセント = 100;

            テスト対象.分割領域表示の左上.Should().Be(new Point(100, 100));
        }

        [Fact]
        public void 分割領域表示の左上は左限を反映します()
        {
            テスト対象.分割領域の左限 = 50;
            テスト対象.分割領域の上限 = 100;
            テスト対象.元画像倍率パーセント = 100;

            テスト対象.分割領域表示の左上.Should().Be(new Point(50, 100));
        }

        [Fact]
        public void 分割領域表示の左上は上限を反映します()
        {
            テスト対象.分割領域の左限 = 100;
            テスト対象.分割領域の上限 = 50;
            テスト対象.元画像倍率パーセント = 100;

            テスト対象.分割領域表示の左上.Should().Be(new Point(100, 50));
        }

        [Fact]
        public void 分割領域表示の左上は倍率パーセントを反映します()
        {
            テスト対象.分割領域の左限 = 100;
            テスト対象.分割領域の上限 = 100;
            テスト対象.元画像倍率パーセント = 50;

            テスト対象.分割領域表示の左上.Should().Be(new Point(50, 50));

        }

        [Fact]
        public void 分割領域表示のサイズは取得できます()
        {
            テスト対象.分割領域の左限 = 100;
            テスト対象.分割領域の右限 = 200;
            テスト対象.分割領域の上限 = 100;
            ...
            テスト対象.分割領域表示のサイズ.Should().Be(new Size(100, 100));
        }

        [Fact]
        public void 分割領域表示のサイズは倍率パーセントを反映します()
        {
            テスト対象.分割領域の左限 = 100;
            テスト対象.分割領域の右限 = 200;
            テスト対象.分割領域の上限 = 100;
            ...
            テスト対象.分割領域表示のサイズ.Should().Be(new Size(200, 200));
        }
        [Fact]
        public void 分割領域表示のサイズは左限を反映します()
        {
            テスト対象.分割領域の左限 = 50;
            テスト対象.分割領域の右限 = 200;
            テスト対象.分割領域の上限 = 100;
            ...
            テスト対象.分割領域表示のサイズ.Should().Be(new Size(150, 100));
        }
        [Fact]
        public void 分割領域表示のサイズは右限を反映します()
        {
            テスト対象.分割領域の左限 = 100;
            テスト対象.分割領域の右限 = 250;
            テスト対象.分割領域の上限 = 100;
        ...
        [Fact]
        public void 分割領域表示のサイズは上限を反映します()
        {
            テスト対象.分割領域の左限 = 100;
            テスト対象.分割領域の右限 = 200;
            テスト対象.分割領域の上限 = 50;
            ...
            テスト対象.分割領域表示のサイズ.Should().Be(new Size(100, 150));
        }
        [Fact]
        public void 分割領域表示のサイズは下限を反映します()
        {
            テスト対象.分割領域の左限 = 100;
            テスト対象.分割領域の右限 = 200;
            テスト対象.分割領域の上限 = 100;
